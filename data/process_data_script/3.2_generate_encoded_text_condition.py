...
def process_description(
    ...
    for s in tqdm(range(0, len(pure_sentences), batch_size), desc="Encoding sentences"):
        ...
        for idx, path in enumerate(save_path):
            print('[Write]', path)
            np.save(path, dict(encoded_text=encoded_text[idx, ...], text=input_text[idx]))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Encode text conditions with T5.')
    parser.add_argument('--description_path', type=Path, default=Path('../datasets/3_text_condition'))
    ...
    parser.add_argument('--max_sentence_length', type=int, default=512)
    ...
    description_path.mkdir(exist_ok=True, parents=True)

    encoded_text_path = args.encoded_text_path
    if args.reset_output:
        shutil.rmtree(encoded_text_path, ignore_errors=True)
    encoded_text_path.mkdir(exist_ok=True, parents=True)
