...
if __name__ == '__main__':
    ...
    train_dataset = DiffusionDataset(dataset_path=Path(d_configs['dataset_path']), include_shape_ids=train_shape_ids)
    val_dataset = DiffusionDataset(
        dataset_path=Path(d_configs['dataset_path']),
        include_shape_ids=val_shape_ids if val_shape_ids is not None else train_shape_ids,
    )
    ...
    val_dataloader = DataLoader(val_dataset,
                        num_workers=d_configs['n_workers'], batch_size=d_configs['batch_size'],
                        drop_last=False, shuffle=False, pin_memory=True, persistent_workers=True)
    config['evaluation']['sdf_model_path'] = Path(train_dataset.get_gensdf_ckpt_path())
    ...
    checkpoint_kwargs = {
        ...
        'filename': config['checkpoint'].get('filename', "diffusion-{epoch:04d}"),
    }
    if config['checkpoint'].get('monitor'):
        ...
        checkpoint_kwargs['filename'] = config['checkpoint'].get('filename', "diffusion-{epoch:04d}-{val_loss:.5f}")
