...
from .adaptive_object_gated_multimodal_diffusion import (
    GatedCrossAttentionBlock,
    SequenceConditionEncoder,
    _masked_mean,
    gather_object_sequence,
)
...
class AdaptiveObjectRoutedLatentDenoiser(nn.Module):
    def __init__(self, config, num_functions: int):
        ...
        self.text_blocks = nn.ModuleList(
            [GatedCrossAttentionBlock(self.hidden_dim, n_heads, ffn_dim, dropout) for _ in range(expert_depth)]
        )
        self.image_blocks = nn.ModuleList(
            [GatedCrossAttentionBlock(self.hidden_dim, n_heads, ffn_dim, dropout) for _ in range(expert_depth)]
        )
        self.fusion_blocks = nn.ModuleList(
            [GatedCrossAttentionBlock(self.hidden_dim, n_heads, ffn_dim, dropout) for _ in range(fusion_depth)]
        )
    ...
    def _encode_modality(self, encoder, values, availability, mask, null_token, null_pooled):
        sequence, has_any = gather_object_sequence(values, availability, mask)
        encoded_tokens, encoded_pooled = encoder(sequence.float())
        keep = has_any[:, None, None]
        tokens = torch.where(
            keep,
            encoded_tokens,
            null_token.expand(encoded_tokens.shape[0], encoded_tokens.shape[1], -1),
        )
        pooled = torch.where(has_any[:, None], encoded_pooled, null_pooled.expand(encoded_pooled.shape[0], -1))
        return tokens, pooled, has_any

    def _run_conditioned_blocks(
        self,
        state: torch.Tensor,
    ...
    def forward(self, x_t, timesteps, batch):
        ...
        base_state = self.backbone(tokens, src_key_padding_mask=key_padding_mask)

        false_flags = torch.zeros_like(has_text, dtype=torch.bool)
        text_state = self._run_conditioned_blocks(
            base_state.clone(),
            self.text_blocks,
            text_tokens,
            self.null_image_token.expand(image_tokens.shape[0], image_tokens.shape[1], -1),
            text_pooled,
            self.null_image_pooled.expand(image_pooled.shape[0], -1),
            has_text,
            false_flags,
            key_padding_mask,
        )
        image_state = self._run_conditioned_blocks(
            base_state.clone(),
            self.image_blocks,
            self.null_text_token.expand(text_tokens.shape[0], text_tokens.shape[1], -1),
            image_tokens,
            self.null_text_pooled.expand(text_pooled.shape[0], -1),
            image_pooled,
            false_flags,
            has_image,
            key_padding_mask,
        )
        fused_state = self._run_conditioned_blocks(
            base_state.clone(),
            self.fusion_blocks,
            text_tokens,
            image_tokens,
        ...
        uncond_hidden = base_state[:, 1:, :]
        text_hidden = text_state[:, 1:, :]
        image_hidden = image_state[:, 1:, :]
        fused_hidden = fused_state[:, 1:, :]

        return {
            "unconditional": self.uncond_head(uncond_hidden, function_id),
            "text": self.text_head(text_hidden, function_id),
            "image": self.image_head(image_hidden, function_id),
            "text_image": self.fused_head(fused_hidden, function_id),
        }


class AdaptiveObjectRoutedMultimodalFunctionAwareDiffusion(TransArticulatedBaseModule):
    def __init__(self, config):
        ...
        self.denoiser = AdaptiveObjectRoutedLatentDenoiser(config, self.num_functions)
    ...
    def register_branch_function_loss_weights(self, overrides):
        branch_names = ["unconditional", "text", "image", "text_image"]
    ...
    def _branch_weights(self):
        ...
        return {
            "unconditional": float(loss_config.get("unconditional_weight", 0.15)),
            "text": float(loss_config.get("text_weight", 0.70)),
            "image": float(loss_config.get("image_weight", 0.70)),
            "text_image": float(loss_config.get("text_image_weight", 0.35)),
            "agreement": float(loss_config.get("agreement_weight", 0.05)),
        }
    ...
    def training_step(self, batch, batch_idx):
        ...
        losses = {
            branch_name: self._masked_loss(preds[branch_name], noise, mask, batch["function_id"].long(), branch_name)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        x_t = self.q_sample(x0, t, noise)
        preds = self.denoiser(x_t, t, batch)
        val_loss = self._masked_loss(preds["text_image"], noise, mask, batch["function_id"].long(), "text_image")
    ...
    def _predict_noise_cfg(self, x_t, t, batch, branch_name: str, guidance_scale: float):
        preds = self.denoiser(x_t, t, batch)
        if branch_name == "unconditional" or guidance_scale == 1.0:
            return preds[branch_name]
        uncond = preds["unconditional"]
        cond = preds[branch_name]
